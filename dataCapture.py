...
class Capture():
    ...
    def saveQfig(self, root_path="/home/photon/data/"):
        """
        Save Qfig data as .png of qfunc and .npy of array of
        complex max values of fft's
        Creates a folder with today's date: 01-13-2014
        If folder for today doesn't exist, create it; else
    ...
    # Save all data as an np array in the default
    # directory or wherever specified
    def saveRaw(self, root_path="/home/photon/data/"):
        ...
        filename = datetime.datetime.now().strftime("%H-%M-%S") + "_raw"
    ...
    def takeData(self, N=50, M=20, ROI=[500, 900]):
        # N is frame request size sent to the ZMQ server;
        # M is rounds of data (total shots is N*M)
        # The pixel range for FFT analysis (Region Of Interest):
        minPixel = ROI[0]
        maxPixel = ROI[1]
